...
class intro(Scene):
    ...
    # Brute Force
        ...
        _cir_trans = Circle(radius=1, color="#ffffff", fill_color="#31355a", fill_opacity=1).scale(0.1)
        self.add(_cir_trans)
        self.play(GrowFromCenter(_cir_trans), _cir_trans.animate.scale(100), run_time=3)
        ...
        big_o_expression = Tex("$O(m) + O(n)$", color="#ffffff").scale(2.5).shift(LEFT)
        big_o_expression_ds = big_o_expression.copy().set_color(BLACK).shift(DOWN * 0.025 + LEFT * 0.025).set_z_index(-1)
        ...
        big_o_expression_n = Tex("O(n)", color="#ffffff").scale(2.5).shift(LEFT)
        big_o_expression_ds_n = big_o_expression_n.copy().set_color(BLACK).shift(DOWN * 0.025 + LEFT * 0.025).set_z_index(-1)

        self.play(Transform(big_o_expression, big_o_expression_n), Transform(big_o_expression_ds, big_o_expression_ds_n), run_time=1)

        self.wait(5)

        self.play(*[FadeOut(mobject) for mobject in self.mobjects], run_time=2)
